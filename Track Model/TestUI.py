...
class TestUI(QtWidgets.QMainWindow):
    ...
    def tempChanged(self):
        ...
        signals.trackModelGUITempSignal.emit(int(self.tempEntry.text()))
    ...
    def changeCrossingStatuses(self):
        if self.RedXing.isChecked() and self.GreenXing.isChecked():
            signals.trackModelGUICrossingChanged.emit(1) 
        elif self.RedXing.isChecked() and not self.GreenXing.isChecked():
            signals.trackModelGUICrossingChanged.emit(2)
        elif not self.RedXing.isChecked() and self.GreenXing.isChecked():
            signals.trackModelGUICrossingChanged.emit(3)
        else:
            signals.trackModelGUICrossingChanged.emit(4)
    ...
    def setOcc(self):
        signals.trackModelGUIOccupancyPressed.emit(self.OccLineSel.currentText(), self.OccBlockSel.currentText())
        
    def setVac(self):
        signals.trackModelGUIVacancyPressed.emit(self.OccLineSel.currentText(), self.OccBlockSel.currentText())

    def changeSwitchOpt1(self):
        signals.trackModelGUISwitchChanged.emit(self.SwitchLineSelect.currentText(), self.SwitchBlockSelect.currentText(), self.SwitchOption1.text())

    def changeSwitchOpt2(self):
        signals.trackModelGUISwitchChanged.emit(self.SwitchLineSelect.currentText(), self.SwitchBlockSelect.currentText(), self.SwitchOption2.text())

    def induceFault(self, faultType):
        # emit signal
        signals.trackModelGUIFaultSignal.emit(self.FaultLineSelect.currentText(), self.FaultBlockSelect.currentText(), faultType)
